...
## Non-negotiables
1. Speed first. Target Lighthouse 100 across the board on mobile. **Our** client JS **on page load** stays under 10KB: an app's code is fetched when its window opens, and does not count until then. Measure with `python3 tools/js-budget.py` after `npm run build`, never by eye. Nothing loads on page load that the visitor did not ask for (audio, video, embeds, third-party scripts), with **one named exception**: the Cloudflare Web Analytics beacon, 30,294 bytes and 10,125 gzipped, added deliberately by Yves who was shown the numbers first. It is cookieless. It reaches the page one of two ways and never both, because two beacons count every visit twice: Cloudflare's automatic setup injects it at the edge, which is what is switched on today, or `site.analyticsToken` is filled in and we render it ourselves with `defer`. `site.analytics` is the separate flag for whether anything is counting at all, and it is what the privacy note in About this site reads. The budget tool counts it on its own line so it can never quietly buy room for more of ours. A second third-party script is a fresh decision, not a precedent
