...
## Non-negotiables
1. Speed first. Target Lighthouse 100 across the board on mobile. **Our** client JS **on page load** stays under 10KB: an app's code is fetched when its window opens, and does not count until then. Measure with `python3 tools/js-budget.py` after `npm run build`, never by eye. Nothing loads on page load that the visitor did not ask for (audio, video, embeds, third-party scripts), with **one named exception**: the Cloudflare Web Analytics beacon, 30,294 bytes and 10,125 gzipped, added deliberately by Yves who was shown the numbers first. It is cookieless, and **there is no analytics script in this repo**: Cloudflare's automatic setup injects it at the edge, version pinned and carrying a Subresource Integrity hash, which a hand embedded snippet cannot safely have. Do not add one as well, that counts every visit twice. `site.analytics` records that counting is happening and is what the privacy note in About this site and the budget tool both read. The budget tool counts it on its own line so it can never quietly buy room for more of ours. A second third-party script is a fresh decision, not a precedent
